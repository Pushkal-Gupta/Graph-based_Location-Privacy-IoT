...
def create_city_graph(grid_size, bounds):
    ...
    with open('city_graph.json', 'w') as f:
...
def load_and_map_trajectories(data_dir, grid_size, bounds, max_users):
    ...
    records = []
    user_dirs = sorted(os.listdir(data_dir))[:max_users] if max_users else sorted(os.listdir(data_dir))
    ...
    df_locations = df_locations.drop_duplicates(
        subset=["user_id","location_id","timestamp"]
    )
    
    df_locations = df_locations.sort_values(["user_id","timestamp"])
    
    df_locations.to_csv("device_locations.csv", index=False)
    
    print(f"Trajectory dataset created: {len(df_locations)} records")

if __name__ == "__main__":
    print("Preprocessing GeoLife...")
